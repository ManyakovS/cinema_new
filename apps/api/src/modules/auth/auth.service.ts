...
import { LoginUserDto, CreateVerifyDto, RegisterUserDto, VerifyDto } from './dto/auth.dto'
  ...
  async sendVerifyCode(body: CreateVerifyDto) {
    ...
    if (!exitingUser) {
      ...
      await this.emailService.sendEmail({
        to: body.email,
        subject: 'Верификация',
        body: getEmailVerifyTemplate({ code, link: `${process.env.CLIENT_HOST}/auth/register/register/main-form?email=${body.email}&verifyCode=${code}` }),
      })
